...
# One with raw data and one with O3 (8 hour maximum)
# Max Temperature
import numpy as np
import pandas as pd
...
#t_import = 'UROP_files2/Tem-Wind-P-RH/'
pblhlist = []
radlist = []
metlist = []
tlist = []
...
#for files in glob.glob(t_import + '*.nc'):
#    tlist.append(files)
#print(len(pblhlist), len(radlist), len(metlist), len(tlist))
...
#     nc = netCDF4.Dataset(tlist[i], mode='r')
#     t = nc.variables['T10M'][0]
#     time_var = nc.variables['time']
#     dtime = netCDF4.num2date(time_var[:][-1],time_var.units)
#     timestamp_t.append(dtime)
#     t_data.append([dtime, t])
...
# Sorting Temperature data according to time
#t_data = sorted(t_data,key=lambda x: (x[0],x[1]))
#timestamp_t = sorted(timestamp_t)
...
# adjust length of temperature data
#timestamp_t_adj = timestamp_t
#t_data_adj = t_data

#print(met_data[0][1][30][30])
#print(len(timestamp_met_adj), len(timestamp_pblh_adj), len(timestamp_rad_adj), len(timestamp_t_adj))
...
header = ['Timestamp', 'FSPMC', 'RSPMC', 'O3','O3_8hMax', 'pressure', 'humidity', 'temperature', 'wind speed', 'PBLH', 'RAD']
...
for f in range(len(filelist) + 1):
    ...
    if f==len(filelist):
        ...
        print(station[0], latitude[0], longitude[0], values[1][2], unit) #[time][station]
        print('Total stations:', len(station), '; Total number of days:')
        print("\n")

    if f==1 or f==2 or f==4 or f==len(filelist): #FSPMC, RSPMC, O3, O3_8hmax
        ...
        for s in range(len(station)):
            ...
            first_index = 0
            if (len(delete_index) != 0) and (len(non_delete) != 0):
            ...
            if minimum_len < 0.2*len(data): # negative values smaller than 20% of the whole data
                ...
                csvfile = directory + station[s] + '('+ str(lon) + ';' + str(lat) + ').csv'
                ...
                ##### 2.
                # Check whether the raw csv file exists
                csvfile2 = directory + station[s] + '('+ str(lon) + ';' + str(lat) + ')_raw.csv'

                # if exists, import
                if os.path.exists(csvfile2):
                    df2 = pd.read_csv(csvfile2)
                    df2 = np.array(df2)
                    df2 = df2[:,1:] # Delete the first column with index number 0, 1, 2,,

                # if not, make new one
                else:
                    df2 = [[0 for x in range(len(header))] for y in range(len(timestamp)+1)]
                    df2[0] = header
                    df2 = np.array(df2)
                    df2[:,0][1:] = timestamp
                ...
                if f==1:
                    ...
                    df2[:,1][1:][first_index:first_index+len(data)] = data
                if f==2:
                    df[:,2][1:][first_index:first_index+len(baseline)] = baseline
                    df2[:,2][1:][first_index:first_index+len(data)] = data
                if f==4:
                    df[:,3][1:][first_index:first_index+len(baseline)] = baseline
                    df2[:,3][1:][first_index:first_index+len(data)] = data
                if f==len(filelist):
                    df[:,4][1:][first_index:first_index+len(baseline)] = baseline
                    df2[:,4][1:][first_index:first_index+len(data)] = data

                # fill in the meteorologcal values
                df[:,5][1:][first_index:first_index+len(ps_bl)] = ps_bl
                df[:,6][1:][first_index:first_index+len(rh_bl)] = rh_bl
                df[:,7][1:][first_index:first_index+len(temp_bl)] = temp_bl
                df[:,8][1:][first_index:first_index+len(ws_bl)] = ws_bl
                df[:,9][1:][first_index:first_index+len(pblh_bl)] = pblh_bl
                df[:,10][1:][first_index:first_index+len(rad_bl)] = rad_bl
                
                df2[:,5][1:][first_index:first_index+len(ps)] = ps
                df2[:,6][1:][first_index:first_index+len(rh)] = rh
                df2[:,7][1:][first_index:first_index+len(t)] = t
                df2[:,8][1:][first_index:first_index+len(ws)] = ws
                df2[:,9][1:][first_index:first_index+len(pblh)] = pblh
                df2[:,10][1:][first_index:first_index+len(rad)] = rad

                # Export
                pd.DataFrame(df).to_csv(csvfile)
                pd.DataFrame(df2).to_csv(csvfile2)
