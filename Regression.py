...
def regression(station):
    ...
    plt.title(filelist[station] + ', R-squared = ' + str(round(rsquared, 4)))
    plt.ylabel("ln(ppb)")
    ...
    ax = plt.axes()  # Create axis
    ax.grid(True)
    plt.plot(dailytime_long, long, label='Long term variation')
    ax.xaxis.set_major_locator(plt.MaxNLocator(5))  # Set Maximum number of x-axis values to show
    fig.autofmt_xdate()  # Rotate values to see more clearly
    plt.legend(loc='best')
    plt.title(title+" Long Term Variation")
    plt.ylabel("ln(ppb)")
    fig.savefig(directory + title + " Long Term.png")
    # plt.show()
    plt.close(fig)
    ...
    ax = plt.axes()  # Create axis
    ax.grid(True)
    plt.plot(dailytime_long, season, label='Seasonal term variation')
    ax.xaxis.set_major_locator(plt.MaxNLocator(5))  # Set Maximum number of x-axis values to show
    fig.autofmt_xdate()  # Rotate values to see more clearly
    plt.legend(loc='best')
    plt.title(title + " Seasonal Variation")
    plt.ylabel("ln(ppb)")
    fig.savefig(directory + title + " Seasonal Term.png")
    # plt.show()
    plt.close(fig)
    ...
    ax = plt.axes()  # Create axis
    ax.grid(True)
    plt.plot(dailytime, short, label='Short term variation')
    ax.xaxis.set_major_locator(plt.MaxNLocator(5))  # Set Maximum number of x-axis values to show
    fig.autofmt_xdate()  # Rotate values to see more clearly
    plt.legend(loc='best')
    plt.title(title + " Short Term Variation")
    plt.ylabel("ln(ppb)")
    fig.savefig(directory + title + " Short Term.png")
    # plt.show()
    plt.close(fig)

    # Write csv file of result matrix
    with open(directory + title + '.csv', "w") as output:
